...
public:
    ...
    bool WriteZCSBITSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZCSBITSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZCSBITSeed_deprecated(uint256& seed);
    bool EraseZCSBITSeed();
    bool EraseZCSBITSeed_deprecated();

    bool WriteZCSBITCount(const uint32_t& nCount);
    bool ReadZCSBITCount(uint32_t& nCount);
