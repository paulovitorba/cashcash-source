...
// The zCSBIT block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zCSBIT is a hash of the serial
...
{
    //Create an output returning the zCSBIT that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZCSBITOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zCSBIT output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zCSBIT", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZCSBITOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zCSBIT output", __func__);
...
{
    CzCSBITTracker* zcsbitTracker = pwallet->zcsbitTracker.get();
    CMintMeta meta;
    if (!zcsbitTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zcsbitTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a CSBIT stake is the outpoint
