...
BOOST_AUTO_TEST_CASE(zerocoin_wrapped_serial_spend_test)
{
    ...
    CzCSBITWallet *czCSBITWallet = new CzCSBITWallet(wallet.strWalletFile);
    ...
    for (unsigned int i = 0; i < TESTS_COINS_TO_ACCUMULATE; i++) {
        ...
        czCSBITWallet->GenerateDeterministicZCSBIT(denom, coin, dMint, true);
        czCSBITWallet->UpdateCount();
