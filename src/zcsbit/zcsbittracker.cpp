...
CzCSBITTracker::CzCSBITTracker(std::string strWalletFile)
{
...
CzCSBITTracker::~CzCSBITTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzCSBITTracker::Init()
{
...
bool CzCSBITTracker::Archive(CMintMeta& meta)
{
...
bool CzCSBITTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzCSBITTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzCSBITTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzCSBITTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzCSBITTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzCSBITTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzCSBITTracker::GetSerialHashes()
{
...
CAmount CzCSBITTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzCSBITTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzCSBITTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzCSBITTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzCSBITTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzCSBITTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzCSBITTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzCSBITTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzCSBITTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzCSBITTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzCSBITTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzCSBITWallet* zCSBITWallet)
{
    bool iszCSBITWalletInitialized = (NULL != zCSBITWallet);
    CMintMeta meta;
    ...
    if (! iszCSBITWalletInitialized)
        zCSBITWallet = new CzCSBITWallet(strWalletFile);
    meta.isSeedCorrect = zCSBITWallet->CheckSeed(dMint);
    if (! iszCSBITWalletInitialized)
        delete zCSBITWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzCSBITTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzCSBITTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzCSBITTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzCSBITTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzCSBITTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzCSBITTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzCSBITWallet* zCSBITWallet = new CzCSBITWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zCSBITWallet);
        }
        delete zCSBITWallet;
        LogPrint("zero", "%s: added %d dzcsbit from DB\n", __func__, listDeterministicDB.size());
    }
