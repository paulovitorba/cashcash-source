...
CzCSBITWallet::CzCSBITWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zcsbit seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZCSBITSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZCSBITSeed_deprecated()) {
                    LogPrintf("%s: Updated zCSBIT seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zcsbit seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zcsbit wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzCSBITWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZCSBITCount(nCountLastUsed);
    else if (!walletdb.ReadZCSBITCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzCSBITWallet::Lock()
{
    seedMaster = 0;
}

void CzCSBITWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzCSBITWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZCSBIT(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzCSBITWallet::LoadMintPoolFromDB()
{
...
void CzCSBITWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzCSBITWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzCSBITWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zcsbitTracker->HasPubcoinHash(pMint.first)) {
...
bool CzCSBITWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZCSBIT(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zcsbitTracker which also adds to database
    pwalletMain->zcsbitTracker->Add(dMint, true);
...
void CzCSBITWallet::SeedToZCSBIT(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzCSBITWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzCSBITWallet::UpdateCount()
{
    nCountLastUsed++;
    CWalletDB walletdb(strWalletFile);
    walletdb.WriteZCSBITCount(nCountLastUsed);
...
void CzCSBITWallet::GenerateDeterministicZCSBIT(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzCSBITWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzCSBITWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzCSBITWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
