...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* blockindex, bool txDetails = false)
{
    ...
    UniValue zcsbitObj(UniValue::VOBJ);
    for (auto denom : libzerocoin::zerocoinDenomList) {
        zcsbitObj.push_back(Pair(to_string(denom), ValueFromAmount(blockindex->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    zcsbitObj.push_back(Pair("total", ValueFromAmount(blockindex->GetZerocoinSupply())));
    result.push_back(Pair("zCSBITsupply", zcsbitObj));
...
UniValue getchecksumblock(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw runtime_error(
            ...
            "  \"zCSBITsupply\" :\n"
            "  {\n"
            "     \"1\" : n,            (numeric) supply of 1 zCSBIT denomination\n"
            "     \"5\" : n,            (numeric) supply of 5 zCSBIT denomination\n"
            "     \"10\" : n,           (numeric) supply of 10 zCSBIT denomination\n"
            "     \"50\" : n,           (numeric) supply of 50 zCSBIT denomination\n"
            "     \"100\" : n,          (numeric) supply of 100 zCSBIT denomination\n"
            "     \"500\" : n,          (numeric) supply of 500 zCSBIT denomination\n"
            "     \"1000\" : n,         (numeric) supply of 1000 zCSBIT denomination\n"
            "     \"5000\" : n,         (numeric) supply of 5000 zCSBIT denomination\n"
            "     \"total\" : n,        (numeric) The total supply of all zCSBIT denominations\n"
            "  }\n"
            "}\n"
...
UniValue getaccumulatorwitness(const UniValue& params, bool fHelp)
{
    ...
    if (!GenerateAccumulatorWitness(pubCoin, accumulator, witness, nMintsAdded, strFailReason)) {
        receipt.SetStatus(_(strFailReason.c_str()), ZCSBIT_FAILED_ACCUMULATOR_INITIALIZATION);
