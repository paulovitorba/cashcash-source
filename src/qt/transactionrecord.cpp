...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.HasZerocoinSpendInputs()) {
        ...
        if (wtx.HasZerocoinPublicSpendInputs()) {
            ...
            if (!ZCSBITModule::ParseZerocoinPublicSpend(wtx.vin[0], wtx, state, publicSpend)){
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.HasZerocoinSpendInputs() && (fZSpendFromMe || wallet->zcsbitTracker->HasMintTx(hash))) {
            //zCSBIT stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::StakeZCSBIT;
...
bool IsZCSBITType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakeZCSBIT:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::StakeZCSBIT || type == TransactionRecord::MNReward) {
