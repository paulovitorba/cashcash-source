...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 CSBIT.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
static void NotifyzCSBITReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzCSBITReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzCSBITReset.connect(boost::bind(NotifyzCSBITReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzCSBITReset.disconnect(boost::bind(NotifyzCSBITReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zcsbitTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus, fWrongSeed);
}
