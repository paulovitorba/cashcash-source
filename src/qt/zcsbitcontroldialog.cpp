...
#include "ui_zcsbitcontroldialog.h"
...
void ZPivControlDialog::updateList()
{
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        {
            LOCK(pwalletMain->zcsbitTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zcsbitTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zCSBIT.";
            else if (!mint.isSeedCorrect)
                strReason = "The zCSBIT seed used to mint this zCSBIT is not the same as currently hold in the wallet";
            else
